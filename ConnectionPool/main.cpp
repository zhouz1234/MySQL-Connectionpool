...
void op1(int begin, int end)
{
    ...
    {
        MysqlConn conn;
        conn.connect("root", "123456", "students", "127.0.0.1");
        char sql[1024] = { 0 };
        sprintf(sql, "insert into stu values(%d,  'man', 'tom')", i);
        conn.update(sql);
    }
}
...
void op2(ConnectionPool* pool, int begin, int end)
{
    for (int i = begin; i < end; ++i)
    {
        shared_ptr<MysqlConn> conn = pool->getConnection();   //取一个链接池；
        char sql[1024] = { 0 };
        sprintf(sql, "insert into stu values(%d, 'man', 'tom')", i);
        conn->update(sql);
    }
...
#else
    //连接池, 多单线程, 用时: 3253066100 纳秒, 3253 毫秒
    ConnectionPool* pool = ConnectionPool::getConnectPool();
    steady_clock::time_point begin = steady_clock::now();
...
int query()
{
    MysqlConn conn;
    conn.connect("root", "123456", "students", "127.0.0.1");
    string sql = "insert into stu values(6,'man', 'tom')";
